#line 1 "e:\\document\\Program Arduino\\jamur-device\\fuzzy.h"
...
void fuzzy(float suhu, float kelembaban) {
  ...
  SC = max(max(rule4, rule5), max(max(rule9, rule10), max(max(rule15, rule20), rule25)));
  C  = max(max(rule1, rule2), max(max(rule3, rule7), max(max(rule8, rule13), max(max(rule14, rule19), rule24))));
  N  = max(max(rule6, rule12), max(rule18, rule23));
  L  = max(max(rule11, rule17), rule22);
  SL = max(rule16,rule21);
