...
const int n = 1;
  ...
  FirebaseData fData ;  
  FirebaseData IDpasien ;
  FirebaseData IDdokter ;
  FirebaseJson json = fData.to<FirebaseJson>();
  String IDP,IDD, pasien, dokter;
  int IDDe;
...
void setup() {
  ...
  Serial.println("---------------------");
Serial.println (Firebase.RTDB.getString(&IDdokter, "/deviceData/11072021015/doctorId"));
while(Firebase.RTDB.getString(&IDpasien, "/deviceData/11072021015/doctorId") == 0 || Firebase.RTDB.getString(&IDdokter, "/deviceData/11072021015/patientId") == 0){
  Serial.print(":......_____.....:");
  
  };

  //Pengambilan ID Dokter dan ID Pasien 
  if (Firebase.RTDB.getString(&IDdokter, "/deviceData/11072021015/doctorId"))
{ 
    
 if (Firebase.RTDB.getString(&IDpasien, "/deviceData/11072021015/patientId"))
  { 
    
   while (IDpasien.to<String>() == "" );
    {

    }
   IDP = IDpasien.to<String>();
  } 
  while (IDdokter.to<String>() == "" );
  {
  
  }
  Serial.print (" ID PAsien ");
  Serial.println (IDP);
  Serial.print (" ID DOkter ");
  Serial.println (IDD);
  
 IDD = IDdokter.to<String>();
}
  ...
  if (irValue > FINGER_ON ) {
    display.clearDisplay();
     display.setTextSize(1);                                   //Near it display the average BPM you can display the BPM if you want
      ...
      display.println(data_filtered[n],1); 
...
if ((millis() - sendDataPrevMillis > 15000 || sendDataPrevMillis == 0))
  {
    ...
    data_filtered[n] = alpha * tambah + (1 - alpha) * data_filtered[n-1];
    ...
    //IDD (ID Dokter) IDP (ID Pasien)
   Serial.printf("Push json... %s\n", Firebase.RTDB.pushJSON(&fbdo, "/patient/" + IDD + "/" + IDP + "/DataHistory/", &json) ? "ok" : fbdo.errorReason().c_str());
